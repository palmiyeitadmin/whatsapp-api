...
export const onRequestGet = createProtectedRoute(async function(context) {
    ...
    try {
        const result = await env.CF_INFOBIP_DB.prepare(`
            SELECT COUNT(*) as count FROM contacts WHERE user_google_id = ?
        ...
        return new Response(JSON.stringify({ error: 'Failed to fetch contacts count' }), {
            status: 500,
            headers: { 'Content-Type': 'application/json' },
        });
    }
